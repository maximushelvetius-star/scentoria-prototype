...
export default function App() {
  ...
  const handleOpenDuftDesTages = () => {
    ...
    setRecommendationResult({
      primary,
      alternatives,
      selectedData: { mood: 80, weather: "Sonnig", occasion: "Date" }
    });
  ...
  const handleFinishFinder = (
    ...
    setRecommendationResult({ primary, alternatives, selectedData });
    setViewMode("recommendation");
    triggerToast(`Dankeschön! Empfehlung errechnet.`);
  };

  // Rotates to another matching fragrance for the active situation
  const handleRerollRecommendation = () => {
    if (!recommendationResult || !recommendationResult.selectedData) return;
    const { mood, weather, occasion } = recommendationResult.selectedData;
    const { primary, alternatives } = getRecommendation(mood, weather as any, occasion as any);
    setRecommendationResult({
      primary,
      alternatives,
      selectedData: recommendationResult.selectedData,
    });
    triggerToast(`Anderer Duft geladen: ${primary.name}`);
  };
    ...
    if (viewMode === "recommendation" && recommendationResult) {
      return (
        <Recommendation
          ...
          onReroll={handleRerollRecommendation}
          selectedData={recommendationResult.selectedData}
        />
      );
    }
